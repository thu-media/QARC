...
ax.set_ylim([0, 1.5])
ax.set_yticks(np.round(np.linspace(0, 1.5, 6), 1))
# ax.grid(True)
...
p1 = ax.bar((left1[0], left2[0], left3[0], left3[0]), y1,
            bottom=bt, width=width, facecolor=c[0], alpha=0.8, ec='black', lw=8, label=r'QARC')
...
p2 = ax.bar((left1[1], left2[1], left3[1], left3[1]), y2,
            bottom=bt, width=width, facecolor=c[1], ec='black', lw=8, label=r'Google Hangout')
p3 = ax.bar((left1[2], left2[2], left3[2], left3[2]), y3,
            bottom=bt, width=width, facecolor=c[2], ec='black', lw=8, label=r'Compound TCP')
p4 = ax.bar((left1[3], left2[3], left3[3], left3[3]), y4,
            bottom=bt, width=width, facecolor=c[3], ec='black', lw=8, label=r'Vegas')
#plt.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3, prop={'size': 60}, 
#           ncol=4, mode="expand", borderaxespad=0.)
ax.legend(loc="upper right", frameon=True, fontsize=45,
          labelspacing=0.3, handlelength=2.8, borderpad=0.4)
legend = plt.gca().get_legend()
frame = legend.get_frame()
# frame.set_linewidth(0.7)
ax.set_ylabel(r'\textbf{Average Value}')
ax.set_xlabel(r'\textbf{AT\&T LTE Uplink}')
